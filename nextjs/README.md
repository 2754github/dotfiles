...
npm i -D shadcn
npx shadcn init
sed -i '' s'|border"|border border-foreground"|' 'app/[lang]/error.tsx'
sed -i '' s'|border-2|border-2 border-foreground|' 'app/[lang]/error.tsx'
sed -i '' -e 27s"|^|      'shadcn/',\n|" eslint.config.mjs
sed -i '' -e 21s'|^|    "paths": {\n      "@/*": ["./shadcn/*"]\n    },\n|' tsconfig.json
...
npm i @conform-to/valibot@$(jq -r '.dependencies."@conform-to/react"' package.json)
sed -i '' -e 196s"|^|      'react/jsx-handler-names': ['error', { eventHandlerPrefix: 'on' }],\n|" eslint.config.mjs
gaa
gcm 'setup conform (server functions)'
```
