...
alias rm-dsstore="find . -type f -name '.DS_Store' -delete"
...
alias tree="eza -a -I '.git|.memo|.next|build|node_modules|out|vendor' --icons -T -L "
alias f="rg \
  -. \
  -g '!.git/' \
  -g '!*.lock' \
...
gwa () { git fetch && git worktree add $1 $1 && code $1 }
gwr () { git worktree remove --force $(gs | unstaged | sel | awk2) }
