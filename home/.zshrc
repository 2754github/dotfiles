...
alias gcma='GIT_COMMITTER_DATE=$(git log -1 --format=%aI) git commit --amend --no-edit'
...
gld () { git log --format='[%h] %s%n%ai（author:    %an）%n%ci（committer: %cn）%n' -${1:-3} }
...
vsc-go () {
  ...
  gopath=$(go env GOPATH | sed "s|$HOME|~|")
  goroot=$(go env GOROOT | sed "s|$HOME|~|")
  gomod=$(head -1 go.mod | awk2)
  jq "
    .\"go.gopath\" = \"$gopath\"
  | .\"go.goroot\" = \"$goroot\"
  | .gopls.\"formatting.local\" = \"$gomod\"
...
export PATH="$HOME/.local/bin:$PATH" # For Claude Code (Native)
