...
alias rm-dsstore="ls-file | f .DS_Store | xargs -I{} rm -v {}"
...
alias b-up="brew update && brew upgrade && brew upgrade --cask -g && brew cleanup && brew autoremove"
...
gcma () { GIT_COMMITTER_DATE=$(git log -1 --pretty=format:%aI) git commit --amend --no-edit }
...
gdc () {
  ...
  case $1 in
    ...
    *    ) git diff -w $from..$to --diff-filter ad -- ':!*.gen.go' ':!*.lock' ':!*.snap' ':!*.svg' ':!go.sum' ':!package-lock.json' ':!pnpm-lock.yaml' $@ ;;
  esac
}
...
gsub () { git restore -S $(gs | staged | sel | awk2) }
...
vsc-rm-ext () { rm -frv $(ls -d $HOME/.vscode/extensions/* | sel) }
